...
`vals = dict(...)` 의 `llm_key_bidx=bidx(llm_key),` 다음 줄 추가:
```python
        kis_account_no_bidx=bidx(kis_account_no),
```
UPDATE SET 절에 `kis_account_no_bidx=?` 추가, 값 튜플의 `vals["llm_key_bidx"]`
뒤에 `vals["kis_account_no_bidx"]` 추가. INSERT 컬럼리스트에
`kis_account_no_bidx` 추가, VALUES `?` 1개 추가, 값 튜플의
`vals["llm_key_bidx"]` 뒤(`adm` 앞)에 `vals["kis_account_no_bidx"]` 추가.
...
수정(app_key/로컬 LLM 인자 제거, 계좌번호 사용). 파일 내 모든 해당 호출.
...
async def profile_credentials(req: CredsReq, request: Request):
    ...
    if req.llm_key is not None:
        ok, msg = await _validate_local_llm(req.llm_key)
        if not ok:
            raise HTTPException(400, msg)
    auth_store.update_credentials(
        uid, llm_key=req.llm_key, kis_app_key=req.kis_app_key,
...
async function pfSaveCreds(){
  ...
  if(g('pf_or'))b.llm_key=g('pf_or');if(g('pf_ak'))b.kis_app_key=g('pf_ak');
...
App Secret** 2개로 교체(기존 App Key/로컬 LLM 입력 제거). 복구 호출 JS body:
...
계좌번호" + "한국투자증권 App Secret" 2개로 교체(App Key/로컬 LLM 제거).
