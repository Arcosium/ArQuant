...
- [x] **Step 1: Write the test** (replaced hollow regex with AST-based guard — proven by temporary injection)
...
Note: guard is a real guard — proven by temporary injection of `dart_key=req.dart_key,` into the `upsert_user` call, which caused the test to FAIL with the full kwargs list shown in the assertion message, then reverted and confirmed 2 passed. The original regex was hollow (stopped at first `)` from `.strip()`) and would pass even with `dart_key=` present.
...
```bash
git add tests/test_register_no_dart.py docs/superpowers/plans/2026-05-19-arquant-login-overhaul-backend.md
git commit -m "fix(test): robust AST-based guard for dart/label-not-in-upsert (was hollow regex)"
```
