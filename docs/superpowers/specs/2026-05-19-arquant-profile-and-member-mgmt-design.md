...
- 🚪 로그아웃 버튼을 **사람 아이콘 → 프로필 모달**로 대체. 모달에서
  ...
  지시사항 관리를 수행.
- 사장님(hh09080) 단독 ADMIN. ADMIN 전용 "회원 관리"(현황·삭제). 각
  계정의 실거래/모의 여부는 **확인만**(전환 기능 없음).
- 비밀번호 찾기 인증 인자 변경: 3인자 → **한국투자증권 계좌번호 + 한국투자증권 App Secret (2인자)**.

비-목표(out of scope): ADMIN 승격/강등 기능(명시적으로 제거), ADMIN의
회원 실거래/모의 **전환**(확인만, 전환 제거), 다중 ADMIN, Phase 2
멀티테넌트 격리(별도 스펙), UUP 버그(이미 별도 트랙 완료).
...
- **현황(읽기 전용)**: 아이디·생성일·최근 로그인·**실거래/모의 여부**·
  ADMIN 여부. 실거래/모의는 회원 KIS Base URL에서 파생해 *표시만* 한다
  (`kis_broker` 의 `is_mock` 규칙: `"openapivts"` 또는 `":29443"` 포함
  → 모의, 그 외 → 실거래). ADMIN은 전환하지 않고 확인만 한다.
- **회원 삭제**: 완전 삭제. **안전장치 — 본인 계정/단독 ADMIN 자기삭제 차단.**
- 실전/모의 **전환 기능 없음**(사장님 결정: ADMIN은 확인만, 전환 불필요).
- ADMIN 승격/강등: **없음**(요청대로 제외).

엔드포인트(ADMIN 게이트, 비ADMIN 403):
- `GET /api/admin/members` — 현황(실거래/모의 파생 필드 포함, 읽기 전용)
...
- 회원 삭제는 비가역 → 확인 다이얼로그 + 본인/단독ADMIN 자기삭제
  차단 불변식. (실거래/모의는 읽기 전용 표시이므로 변경 위험 없음.)
...
- ADMIN 게이트: 비ADMIN 403, 본인/단독ADMIN 자기삭제 차단, 회원 현황의
  실거래/모의 파생 필드 정확성(is_mock 규칙).
...
4. ADMIN 백엔드 엔드포인트(현황 읽기전용/삭제) + 안전 불변식.
