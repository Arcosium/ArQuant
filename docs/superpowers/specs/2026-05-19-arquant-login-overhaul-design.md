...
- `POST /api/recover_id` — body `{kis_app_key, kis_app_secret, openrouter_key}`.
...
- `POST /api/recover_password` — body `{username, kis_app_key, kis_app_secret,
  openrouter_key, new_password}`. On exact match of all four → set new password
...
HMAC columns:
- New `users` columns: `kis_app_key_bidx`, `kis_app_secret_bidx`, `openrouter_key_bidx` (TEXT).
...
- Lookup: `SELECT username FROM users WHERE kis_app_key_bidx=? AND kis_app_secret_bidx=?
  AND openrouter_key_bidx=?` (+ `username=?` for password reset). No bulk decrypt.
