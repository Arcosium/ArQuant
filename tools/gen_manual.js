...
children.push(
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
    children: [new TextRun({ text: "최종 업데이트: 2026-05-20", size: 20, color: "8A99AB" })] }),
...
children.push(H2("3.4 아이디·비밀번호 찾기 (계정 복구)"));
children.push(P("아이디나 비밀번호를 잊었다면, 로그인 화면의 ‘아이디 찾기’ / ‘비밀번호 찾기’를 사용합니다. 본인만 알 수 있는 2가지 값으로 본인을 확인합니다 — 가입 시 입력한 한국투자증권 계좌번호와 KIS App Secret입니다(이메일·문자 인증 없음)."));
[
  "아이디 찾기: ‘한국투자증권 계좌번호’ + ‘KIS App Secret’을 입력하면 일치하는 아이디를 알려줍니다.",
  "비밀번호 찾기(재설정): ‘아이디’ + ‘계좌번호’ + ‘KIS App Secret’이 모두 맞으면 새 비밀번호(10자 이상 + 특수문자 1개)를 바로 설정합니다.",
].forEach((t) => children.push(bullet(t)));
children.push(note("중요", "복구는 ‘계좌번호 + App Secret’ 2가지로만 가능합니다. 이 두 값(특히 App Secret)을 안전하게 보관하세요. 둘 다 모르면 계정을 복구할 수 없습니다. 시도 횟수에는 제한이 있어 잠시 후 다시 시도해야 할 수 있습니다."));
children.push(note("보안", "API 비밀키는 서버에 Fernet 대칭 암호화로 저장되며 단말로 내려오지 않습니다. 복구는 키를 복호하지 않고 ‘블라인드 인덱스’ 대조로만 확인하므로, 서버도 입력값을 그대로 저장하지 않습니다."));

// 4. 화면 한눈에 보기
children.push(H1("4. 화면 한눈에 보기"));
children.push(P("상단바에는 좌측에 로고, 우측에 💵 API 비용 배지 · 프로필 버튼(내 계정 관리, 13장) · 🚪 로그아웃 버튼, 그리고 실행 상태 배지가 있습니다. 본문은 4개의 탭으로 구성됩니다."));
children.push(table(
  ["탭", "역할"],
  [
...
children.push(note("운용지원실장의 역할 변경(2026-05-20)", "운용지원실장은 이제 ‘코드를 직접 고치거나 서버를 재시작하지 않습니다’. 사이클 결과를 진단해 ‘내 계정(프로필)의 전략 파라미터를 어떻게 조정하면 좋을지’ 제안하는 역할만 합니다. 전략 값 변경은 9장의 ‘전략 커스터마이즈’에서 직접 적용하시면 됩니다."));
...
// 13. 내 계정 관리 · 보안
children.push(H1("13. 내 계정 관리 · 보안"));
children.push(P("상단바의 ‘프로필’ 버튼을 누르면 내 계정 설정 창이 열립니다. 각 항목은 제목을 누르면 펼쳐지는 아코디언 형태입니다. 모든 변경은 보안 감사 기록에 남습니다."));
children.push(H2("13.1 비밀번호 변경"));
children.push(P("현재 비밀번호를 한 번 더 확인한 뒤 새 비밀번호(10자 이상 + 특수문자 1개 이상)로 바꿉니다. 비밀번호는 복호화 불가능한 해시(argon2id)로만 저장됩니다."));
children.push(H2("13.2 API 자격증명 변경"));
children.push(P("OpenRouter Key · KIS App Key/Secret · 한국투자증권 계좌번호 · Base URL을 바꿀 수 있습니다. 바꾸려는 칸만 채우면 되고(빈 칸은 기존 값 유지), 입력한 KIS·OpenRouter 값은 실제 호출로 유효성을 다시 검증합니다. 지금 봇을 돌리고 있는 활성 계정이라면 변경 즉시 런타임에 반영됩니다."));
children.push(note("주의", "복구(아이디·비번 찾기)는 ‘계좌번호 + App Secret’으로 본인을 확인합니다. 이 두 값을 바꾸면 이후 복구 시 새 값으로 입력해야 합니다."));
children.push(H2("13.3 상시 지시사항"));
children.push(P("내 계정에만 적용되는 운용 원칙을 영구적으로 등록해 둘 수 있습니다(예: 매크로 붕괴 대비 방어 자산 우선 등). 운용전략실장이 매 분석 사이클마다 참고합니다. 추가/삭제가 자유로우며, 한 번 삭제한 지시는 서버를 재시작해도 다시 살아나지 않습니다(영구 삭제 보장)."));
children.push(note("안전", "상시 지시는 AI의 ‘참고 지침’으로만 들어가며, 주문을 막는 결정론적 안전 게이트(리스크 한도)는 절대 우회하지 못합니다. 어떤 지시도 위험 한도를 무력화할 수 없습니다."));
children.push(H2("13.4 회원 탈퇴"));
children.push(P("‘회원 탈퇴’를 누르고 비밀번호를 한 번 더 입력하면 계정·세션·저장된 API 자격증명·상시 지시가 모두 영구 삭제됩니다. 되돌릴 수 없습니다. (단, 시스템에 ADMIN이 한 명뿐인 경우를 막기 위해 ADMIN 계정은 탈퇴할 수 없습니다.)"));
children.push(H2("13.5 회원 관리 (관리자 전용)"));
children.push(P("ADMIN 계정으로 로그인한 경우에만 프로필 창에 ‘회원 관리’ 항목이 보입니다. 전체 회원의 아이디·관리자 여부·모의/실거래 모드를 ‘읽기 전용’으로 확인하고, 필요 시 일반 회원을 완전히 삭제할 수 있습니다. 본인 계정과 다른 ADMIN 계정은 안전을 위해 삭제할 수 없습니다."));
children.push(H2("13.6 보안"));
[
  "세션 토큰은 7일 유효하며 서버 재시작 후에도 계정·세션이 유지됩니다.",
  "세션 쿠키는 HttpOnly + Secure + SameSite=Lax로 보호됩니다.",
  "API 비밀키는 서버에 암호화 저장되며 단말로 내려오지 않습니다.",
  "로그인·등록·복구 시도에는 횟수 제한과 감사 기록이 적용됩니다.",
...
const faq = [
  ...
  ["아이디나 비밀번호를 잊었어요.", "로그인 화면의 ‘아이디 찾기 / 비밀번호 찾기’를 쓰세요. 가입 때 입력한 한국투자증권 계좌번호 + KIS App Secret으로 본인을 확인합니다(3.4 참고). 이메일·문자 인증은 없으니 이 두 값을 꼭 보관하세요."],
  ["@운용지원실장에게 설정을 바꿔달라고 했는데 안 바뀌어요.", "2026-05-20부터 운용지원실장은 코드·설정을 직접 바꾸지 않고 ‘진단·제안’만 합니다. 전략 값은 9장 ‘전략 커스터마이즈’에서 직접 적용하세요."],
];
faq.forEach(([q, a]) => {
...
children.push(table(
  ["증상", "조치"],
  [
    ["로그인이 안 됨", "아이디/비밀번호 확인(비번은 10자 이상 + 특수문자 1개). 잊었다면 ‘아이디·비밀번호 찾기’(계좌번호+App Secret)로 복구(3.4)."],
