...
def create_ops_support() -> BaseAgent:
    """운용지원실장 (Ops Support) — 소스코드 수정/에이전트 관리/서버 재시작"""
    return BaseAgent(
        name="운용지원실장",
        role="ops_support",
...
## 안전 규칙 (절대 위반 불가)
1. main_swarm.py의 핵심 트레이딩 로직(_run_analysis_cycle, _build_orders, start_continuous)은 수정 금지
2. config.py의 API 키, 계좌번호 등 민감 정보는 수정 금지
3. infra/kis_broker.py의 주문 실행 로직(place_order, kr_buy, kr_sell)은 수정 금지
4. 수정이 불가능하거나 위험하다고 판단되면 "수정 불가: [사유]"로 답하세요
